...
Ok so in trying to import the quant.sf file from salmon, I realized I did this code all wrong because I basically overwrote the files for each of the samples. So the quant.sf file only has the data from the last Pcli sample. I need to actually make different directories for each sample and then when I use tximport in R, I import the entire list of files for all samples. (i figured this out because I kept unsuccessfully trying to import the quant.sf file into R and the code/manual/tutorial for salmon tximport wasn't making sense at all)
